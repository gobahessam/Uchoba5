...
// تحديث نص رسالة واتساب للتأمين
function updateInsuranceWhatsAppText(lang) {
    const insuranceLink = document.querySelector('.insurance-cta');
    
    const texts = {
        ar: "مرحباً، أرغب في الاستفسار عن خدمة التأمين الطبي للطلاب",
        en: "Hello, I would like to inquire about student medical insurance service",
        ru: "Здравствуйте, я хотел бы узнать о медицинской страховке для студентов"
    };
    
    if (insuranceLink) {
        insuranceLink.href = `https://wa.me/79964072422?text=${encodeURIComponent(texts[lang])}`;
    }
}
...
function updateLanguage(lang) {
    ...
    // تحديث نص روابط WhatsApp
    updateWhatsAppText(lang);
    updateInsuranceWhatsAppText(lang);
...
// إضافة تأثيرات حركية للبطاقة
document.addEventListener('DOMContentLoaded', function() {
    const insuranceCard = document.querySelector('.insurance-card');
    if (insuranceCard) {
        insuranceCard.addEventListener('mousemove', function(e) {
        ...
        insuranceCard.addEventListener('mouseleave', function() {
